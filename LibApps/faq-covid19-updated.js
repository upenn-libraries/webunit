...
jQuery(document).ready(function($){
  ...
  if ($(".block-pennlib-faq-vitale-block").length != 0) {
    // Vitale Lab FAQ
    faqUrl.push("https://web.archive.org/web/20220315145759/https://faq.library.upenn.edu/api/1.0/faqs?iid=707&topic_id=57296");
  }
  if ($(".block-pennlib-faq-covid-block").length != 0) {
    // Covid-19 FAQ
    faqUrl.push("https://web.archive.org/web/20220315145759/https://faq.library.upenn.edu/api/1.0/faqs?iid=707&group_id=8446");
  ...
  var addFaq = function(json, i) {
    ...
    $("#about").append("<dt><button class='dthead closed' tabindex='0' aria-controls='" + faqid + "' aria-expanded='false'>" + question + "</button><div class='Last-updated' aria-hidden='false'><strong>" + 'Last updated: ' + "</strong>" + revdate + "</div></dt>");
      ...
      $.ajax({
        ...
        url: url,
            success: function(json){
              ...
              if (changeOrder) {
						for(var i = 0; i < json.faqs.length; i++) {
							json.faqs[i].order = changeOrder[i+1];
						}
          ...
          $("a.expand").click(function(){
            $(this).parent().parent().children("dl").children("dt").children("button.dthead").each(function(){
              var el = $(this);
              if (el.hasClass("closed")) {
                el.removeClass("closed");
                el.attr("aria-expanded", "true");
          ...
          $("button.dthead").keypress(function (e) {
            var key = e.which;
            if(key == 13) {
              var el = $(this);
              if (el.hasClass("closed")) {
