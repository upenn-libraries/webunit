...
var url = "https://api2.libanswers.com/1.0/faqs?iid=707&topic_id=20127&sort=totalhits&sort_dir=desc&limit=20";
	...
	$.ajax({
		...
		success: function(json){
			for(var i = 0; i < json.data.faqs.length; i++){
				var question = json.data.faqs[i].question;
				var answer = json.data.faqs[i].answer;
                //to remove whitespaces
                answer = answer.replace(/\n/g, "");
                answer = answer.replace(/\r/g,"");
                answer = answer.replace(/&nbsp;/g, "");
                //For the one particular answer that has this string at the end
                answer = answer.replace("Last modified: Friday, 15-Jul-2011, CC","");
                //alert(answer);
